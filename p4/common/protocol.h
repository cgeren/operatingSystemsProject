...
/// Allow user @u (with password @p) to change her password to the new
/// password @n.
///
/// The user name (@u) and both passwords (@p) and (@n) must conform to LEN_UNAME and
/// LEN_PASSWORD.
///
/// @rblock   enc(pubkey, padR("CHANGEPW".aeskey.len(@ablock)))
/// @ablock   enc(aeskey, len(@u).@u.len(@p).@p.len(@n).@n)
/// @response enc(aeskey, "OK").<EOF>       -- Success
///           enc(aeskey, error_code).<EOF> -- Error (see @errors)
///           ERR_CRYPTO.<EOF>              -- Error (see @errors)
/// @errors   ERR_LOGIN       -- @u is not a valid user
///           ERR_LOGIN       -- @p is not @u's password
///           ERR_REQUEST_FMT -- Server unable to extract @u or @p or @n from
///                              request
///           ERR_CRYPTO      -- Server could not decrypt @ablock
const std::string REQ_CPW = "CHANGEPW";
