...
alicenewpass = cse303.UserConfig("alice", "alice_is_still_awesome")
...
print()
cse303.line()
print("Test #3: Overwrites of user password/data")
cse303.line()
server.pid = cse303.do_cmd_a("Restarting server:", [
    "Listening on port "+server.port+" using (key/data) = (rsa, "+server.dirfile+")",
    "Loaded: " + server.dirfile], server.launchcmd())
...
cse303.do_cmd("Changing alice's password.", "___OK___", client.chgP(alice, "alice_is_still_awesome"), server)
cse303.do_cmd("Attempting access with old password.", "ERR_LOGIN", client.persist(alice), server)
cse303.do_cmd("Instructing server to persist data.", "___OK___", client.persist(alicenewpass), server)
cse303.do_cmd("Stopping server.", "___OK___", client.bye(alicenewpass), server)
cse303.await_server("Waiting for server to shut down.", "Server terminated", server)
