...
static vector<pair<string, const char *>> commands = {
    ...
    {REQ_CPW, " -1 [string] Change the user password"},
    ...
    {REQ_KVI, " -1 [string] -2 [file] Insert K/V using file contents"},
    {REQ_KVU, " -1 [string] -2 [file] Upsert K/V using file contents"},
    {REQ_KVG, " -1 [string]           Get value for provided key"},
    {REQ_KVD, " -1 [string]           Delete value for provided key"},
    {REQ_KVA, " -1 [file]             Get all keys, save to a file"}};
...
struct arg_t {
  ...
  arg_t(int argc, char **argv) {
    ...
    string args1[] = {REQ_SET, REQ_GET, REQ_ALL, REQ_CPW, REQ_KVG, REQ_KVD, REQ_KVA};
    string args2[] = {REQ_KVI, REQ_KVU};
    for (auto a : args0) {
      if (command == a) {
        if (arg1 != "" || arg2 != "")
          throw 1;
...
int main(int argc, char **argv) {
  ...
  vector<string> cmd = {REQ_REG, REQ_BYE, REQ_SET, REQ_GET, REQ_ALL, REQ_SAV, REQ_CPW,
                        REQ_KVI, REQ_KVU, REQ_KVG, REQ_KVD, REQ_KVA};
  decltype(req_reg) *func[] = {req_reg, req_bye, req_set, req_get,
                               req_all, req_sav, req_cpw, req_kvi, 
                               req_kvu, req_kvg, req_kvd, req_kva};
